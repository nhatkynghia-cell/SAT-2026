...
>
> **🔴 ƯU TIÊN PHIÊN SAU — CHỈ CÒN VIỆC USER (Claude KHÔNG làm được, đều cần tài khoản):**
> - **(1) DEPLOY VERCEL (blocker lớn nhất, giờ đã sẵn sàng):** Import repo `SAT-2026` → ⚠️ **Root Directory = `sat-prep-web`** (app trong thư mục con) → thêm 4 env:
>   - `NEXT_PUBLIC_SUPABASE_URL` = `https://yynszcfqcvbnuvguwtfy.supabase.co`
>   - `NEXT_PUBLIC_SUPABASE_ANON_KEY` = copy từ `sat-prep-web/.env.local`
>   - `OPENAI_API_KEY` = key hiện tại (user chốt GIỮ để test, thay sau)
>   - `SAT_PREP_SECRET` = sinh mới: `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"` ⚠️ THIẾU biến này → app crash khi khởi động (không fallback).
> - **(2) VỆ SINH BẢO MẬT (sau khi test xong):** thu hồi OpenAI key cũ (file `.anv` ở ROOT chứa key `sk-proj-...` LỘ TRÊN ĐĨA) + reset DB password Supabase (Settings→Database).
> - **(3) [đã xong, giữ để tham chiếu]** ✅ SQL 4 bảng đã chạy (RLS=true cả 4) · ✅ GitHub push + CI XANH (run #2 `verify` Success).
>
> **⏳ AUTHENTICATED VERIFY (cần login uuid thật — Claude KHÔNG có mật khẩu; user login browser giúp thì Claude verify được):** Sau deploy/hoặc local có login: (a) 3 mục phiên này persist THẬT qua Supabase — streak qua reload (bảng `user_progress`), question bank hit-rate > 0 sau vài câu (`questions`), kill-switch cộng dồn qua cold-start (`ai_cost_ledger`); (b) nợ cũ: PvP win-path (login→thắng→leo rank→cap 10 trận/ngày), Mistake variant full-loop (làm biến thể SAI→mastery+SRS box gốc). Account đã confirm: `truongsonht.xd@gmail.com`.
>
