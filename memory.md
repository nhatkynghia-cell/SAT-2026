...
> - **(a) ✅ ĐÃ CHẠY 3 SQL mới + ai_chat_cache (2026-07-02, user xác nhận ảnh):** cả 4 bảng `ai_cost_ledger/questions/user_progress/ai_chat_cache` tồn tại, `rls_enabled=true` cả 4. **Post-migration verify PASS (bảng ĐÃ có, chưa login):** lỗi ghi đổi từ `PGRST205 table not found` → `22P02 invalid uuid "local-default-user"` = PostgREST parse được bảng thật, code chạm đúng, vẫn degrade sạch (streak 91 đọc lại qua file fallback, `/api/admin/ai-cost` 200 costUsd 0). Fail-safe tự tắt khi có bảng. ⏳ Full-loop persist THẬT qua Supabase cần login uuid (Claude không có mật khẩu) — ghi nợ (d).
