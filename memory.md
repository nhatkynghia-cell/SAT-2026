...
>
> **📦 GIT PHIÊN (B):** repo `main` (root ở tầng `10.SAT_Prep_App/`) — **CHƯA COMMIT gì phiên này.** File MỚI: `sat-prep-web/src/lib/{cost-ledger-store,progress-store}.ts` + `sat-prep-web/{ai_cost_ledger,questions,user_progress}.sql`. File SỬA: `sat-prep-web/src/lib/{ai-cost,question-bank}.ts` + `sat-prep-web/src/app/api/{admin/ai-cost,chat,generate-practice,save-data,load-data}/route.ts` + `master_task_list.md` + `memory.md`. Gợi ý commit gọn 1 đơn vị "Phase 1.5: file-based ledgers → Supabase (5.1/2.1/4.1, fail-safe)".
