...
>
> **✅ LÀM XONG:** **58 integration test / 7 file** trong `sat-prep-web/src/test-int/` — task Phase 1 §9.8 (trước `[ ]`). Chạy **THẬT** route+store+pure-logic trên **fake Supabase in-memory**, CI offline không cần DB/creds.
...
> - **🔴 RỦI RO APP phát hiện khi audit (KHÔNG tự sửa — báo user quyết):** `GET /api/migrate-data` upsert coins/xp từ file JSON local, **GET + không auth-gate + không rate-limit + ghi ĐÈ (không merge)** → nếu file tồn tại = vector bơm số dư. Trên prod TRƠ (file gitignored → `fs.existsSync` no-op) nên chưa exploit live, nhưng NÊN khoá/xoá route legacy này trước beta. (Critic cũng lưu payment/create amount-integrity — nhưng đã có test phủ, an toàn.)
> - **🔍 VERIFY:** tsc sạch · test **236/236** (178 unit + 58 integration) · lint 0/0 · build 59 pages. origin/main HEAD `a9348d6`.
>
