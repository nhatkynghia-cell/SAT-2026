...
>
> **🔴 ƯU TIÊN PHIÊN SAU (VIỆC USER — chặn deploy, Claude KHÔNG làm được):**
> - **(a) Chạy 3 SQL mới trên Supabase SQL Editor (DB PROD):** `ai_cost_ledger.sql`, `questions.sql`, `user_progress.sql` (+ `ai_chat_cache.sql` từ 5.3 nếu chưa chạy). SAU KHI CHẠY: cả 3 store tự dùng bảng (fail-safe tự tắt) — nên authenticated-verify 1 vòng (streak persist qua reload; question bank hit-rate > 0 sau vài câu; kill-switch cộng dồn qua cold-start).
> - **(b) Tạo repo GitHub + push `main`** (repo vẫn 0 remote; phiên (A) 3 commit `0e5e810`/`e328498`/`c1fa15d` + phiên (B) CHƯA COMMIT — xem dưới) → CI chạy thật.
> - **(c) Thu hồi OpenAI key cũ + reset DB password Supabase** (từng lộ 2026-06-29).
> - **(d) Authenticated verify** (cần login uuid thật): 3 mục phiên này với BẢNG ĐÃ TẠO (streak/bank/ledger persist thật qua Supabase, không còn fallback file); + nợ cũ PvP win-path & Mistake variant full-loop (xem block (A)).
>
> **🟢 VIỆC CLAUDE TỰ LÀM ĐƯỢC — ĐÃ HẾT 3 mục file-based.** Không còn chỗ file-based nào vỡ trên serverless (5.1/2.1/4.1 xong; economy/mastery/goals/ai-usage/vocab/mistakes/gates đã Supabase từ Nhóm 1). Phiên sau nếu user vẫn chưa làm user-side → cân nhắc: dọn nợ kỹ thuật nhỏ (tier hardcode 'free' §5.2), hoặc chờ user tạo bảng/push repo để authenticated-verify. KHÔNG còn cutover file-based nào cấp thiết.
>
> **📦 GIT PHIÊN (B):** ✅ **ĐÃ COMMIT + PUSH LÊN GITHUB (2026-07-02).** Repo remote: `https://github.com/nhatkynghia-cell/SAT-2026.git` (main). Commit `6c4b156` (5.1/2.1/4.1) + merge README `07f193d` = HEAD, đã verify `origin/main` khớp `07f193d`. **Repo KHÔNG còn "0 remote".** File MỚI: `sat-prep-web/src/lib/{cost-ledger-store,progress-store}.ts` + `sat-prep-web/{ai_cost_ledger,questions,user_progress}.sql`. File SỬA: `sat-prep-web/src/lib/{ai-cost,question-bank}.ts` + `sat-prep-web/src/app/api/{admin/ai-cost,chat,generate-practice,save-data,load-data}/route.ts` + docs. ⚠️ **.gitignore đã cứng hóa 3 lớp chặn secret:** `.env.local`, **`.anv` (file ở ROOT chứa NGUYÊN OpenAI key `sk-proj-...` — LỘ TRÊN ĐĨA, user PHẢI thu hồi key này)**, và `10.SAT_Prep_App - Copy/` (bản Streamlit cũ 2860 file gồm python_embed). Chỉ 23 file sạch lên git. CI `.github/workflows/ci.yml` giờ chạy thật trên push — kiểm ở tab Actions GitHub. ⚠️ **CI run #1 ĐỎ** (2026-07-02): job fail ~10s ở bước `npm ci` — ERESOLVE (`lucide-react@0.344.0` đòi peer react ^16/17/18, dự án dùng react 19). **ĐÃ VÁ commit `2b6aa35`:** `ci.yml` push đầu là bản CŨ (chỉ có `npm ci` + job `test`) — viết lại thành `npm ci --legacy-peer-deps` + khôi phục 3 cổng tsc+test+lint (đúng thiết kế 5.4). Đã reproduce lỗi + verify fix trong temp dir (`npm ci --legacy-peer-deps` exit 0) trước khi push. HEAD sau vá = `2b6aa35`. ⏳ Chờ user xác nhận CI run #2 xanh (Claude không có `gh` CLI để tự xem).
