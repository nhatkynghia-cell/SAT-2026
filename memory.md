...
> - **✅ HỆ SỐ NHÂN XU RPG (commit `ad03bf8`):** `TIER_COIN_MULTIPLIER` (subscription.ts) free 1/premium 1.5/ultimate 2. Thêm tham số `coinMultiplier=1` (backward-compat) vào 5 faucet economy.ts (`applyAnswerReward`/`applyExamRewardFromDifficulties`/`applyQuestReward`/`resolvePvpFight`) — **CHỈ nhân XU, KHÔNG nhân XP**. Wire qua getUserTier ở 4 route (grade/exams-grade/vocab/economy quest+pvp). Spin GIỮ NGUYÊN (RNG may mắn, không phải nỗ lực). +6 test.
> - **🔍 VERIFY:** tsc·test **269/269**·lint 0/0·build **62 pages**. API-verify free (unauth=free fail-safe): score/adaptive/skill-tree khóa đúng, exam real→403 tier, mock→200. Browser DOM `/upgrade`: giá 499k/3.990k·990k/7.990k + copy render đúng.
>
> **🔴 2 QUYẾT ĐỊNH MỚI cuối phiên (user 2026-07-06):** (a) **BỎ HẲN "bonus xu → voucher thi thật"** khỏi định vị Ultimate (voucher = tiền mặt ~2.7tr/user → rủi ro faucet/multi-account không kiểm soát nổi). (b) **THAY bằng GIẢI THƯỞNG TOP-5 học sinh thành tích cao TOÀN QUỐC** — chi phí CỐ ĐỊNH 5 suất, kiểm soát được → **hạng mục NGHIÊN CỨU phiên sau** (tiêu chí xếp hạng chống gian lận + chu kỳ + bảng leaderboard cross-user như parent-report). (c) **AFFILIATE 35% CHỐT ngữ nghĩa:** 35% = tổng ngân sách, chia **KOL 25% hoa hồng + người mua giảm 10%** → giữ 65%. Mỗi mã 2 nút chỉnh (discount%/commission%), mặc định 10/25.
>
> **⏳ VIỆC PHIÊN SAU (đều CHỜ):** (1) 🔬 **NGHIÊN CỨU Top-5 toàn quốc** (leaderboard). (2) 🔨 **TRIỂN KHAI AFFILIATE 35%** — ĐÃ nghiên cứu kỹ thuật đủ (xem CLgia.md): bảng `affiliate_codes`+`affiliate_referrals` + cột `payment_transactions.coupon_code` + áp discount ở `payment/create` + ghi hoa hồng ở IPN sau confirm ok (IPN so amount với `amount_vnd` ĐÃ LƯU nên ghi giá-giảm vào txn+URL cổng là an toàn) + admin page shared-secret. (3) Cổng thanh toán cần creds sandbox VNPay/MoMo (giá ĐÃ chốt). (4) Wave 2 khác: model AI cao cấp cho Ultimate + đề độc quyền (làm Ultimate "đáng gấp đôi"); cap PvP/tower theo tier. (5) Secret rotate GitHub PAT/Vercel/OpenAI vẫn chưa. Chi tiết: memory Claude `sat-prep-funnel-pricing.md` + CLgia.md.
>
> **⚠️ LƯU Ý khác biệt Premium vs Ultimate HIỆN TẠI còn mỏng:** chỉ báo cáo phụ huynh (30d/90d) + hệ số xu (×1.5/×2). Model AI xịn + đề độc quyền mới là chữ trên trang bán, CHƯA code → cân nhắc làm trước khi bán mạnh Ultimate, hoặc Top-5 prize gánh phần "elite".
...
>
> **⏳ VIỆC PHIÊN SAU (thứ tự — vẫn CHỜ user):** (1) 🟡 **CỔNG THANH TOÁN** — cần user (a) chốt giá 4 gói (placeholder 99k/990k/199k/1990k), (b) creds sandbox VNPay (TMN_CODE+HASH_SECRET) + MoMo (PARTNER_CODE+ACCESS_KEY+SECRET_KEY) → verify roundtrip live + field-order chữ ký IPN MoMo. (2) ⏳ **Beta 100 users** (pháp lý trẻ vị thành niên + tuyển). (3) **Secret rotate:** GitHub PAT `ghp_...HETIG` / Vercel token / OpenAI key vẫn CHƯA đổi (chỉ DB pw) → HỎI lại + tick. ⚠️ nếu rotate git/Vercel token → token cũ hết hiệu lực, xin token mới.
> **✅ ĐÃ RÀ (2026-07-06 cuối phiên): 2 "nice-to-have" cũ ĐÃ XONG TỪ TRƯỚC — memory cũ ghi sai:** (a) **panel xu hướng tuần cho học sinh ở /dashboard** = commit `27651d8` (route `/api/progress/weekly` đọc `loadOwnSnapshots` RLS + `WeeklyTrendPanel.tsx` dùng chung học-sinh/phụ-huynh + wire dashboard dòng 167). (b) **UI lịch sử đổi quà học-sinh** = `shop/page.tsx:124-129` section "🎟️ Lịch sử đổi quà" (fetch `/api/redeem` GET, render trạng thái phiếu). → **KHÔNG còn nice-to-have code độc lập nào Claude tự làm được; mọi việc còn lại đều CHỜ user (giá/creds/beta/secret) hoặc là scope LỚN cần user đồng ý (Nhóm 7 Phase-2: #1 streak đối thủ ảo, #2 câu vàng hằng ngày, #3 loss-aversion, #7 pacing — đừng tự mở).** Nhỏ còn hở: master_task_list 3.3 empty-state (phần lớn dashboard đã có empty-state tốt) + UI admin chưa browser click-verify.
...
> - **🔑 KIẾN TRÚC HARNESS (tái dụng cho mọi integration test route sau này):** `node --test` KHÔNG resolve `@/` + `next/server`/`server-only` không bare-import được (đã spike xác nhận). Giải: **resolve-hook loader** (`loader.mjs` đăng ký qua `loader-register.mjs` bằng `module.register`) remap `@/`→`src/` + stub `next/server` (NextResponse=class kế thừa Response + static json) / `server-only` (no-op) / `@/lib/supabase/{server,admin}`→fake client. `fake-db.mjs` mô phỏng query-builder + 3 RPC atomic (`redeem_reward`/`confirm_payment`/`consume_pvp_fight`) theo đúng SQL, **deep-clone (`structuredClone`) ở MỌI ranh giới đọc/ghi** (mấu chốt fidelity — giống PostgREST trả JSON mới; mutate object trả về KHÔNG đụng "DB", chỉ UPDATE/UPSERT round-trip mới persist). Wire vào `npm test` (chạy CẢ unit .ts + integration .mjs). 0 file production đụng vào.
> - **Phủ bất biến money-safety:** grade/exams-grade (chấm server, cộng 1 lần, replay chặn, **combo streak cap 1.5×**, ownership) · economy (`answer`/`exam`→400, quest double-claim→409, spend, spin, **PvP win-path reward đúng rank kế / cap 10 trận / targetRank client BỊ BỎ QUA / fail-safe pre-migration**) · redeem (giá REWARDS bỏ qua `cost` client, fail-closed 503, **cross-user isolation + no_row**) · vocab (farm chặn cần due+persist, box Leitner) · **payment IPN momo+vnpay** (CHỈ chữ ký hợp lệ mới cấp gói, **idempotent chống double-grant**, sai tiền chặn; VNPay ký THẬT HMAC-SHA512 bằng lib, amount ÷100) · payment/create (giá PLANS bỏ qua `amount` client, auth 401, validate).
> - **🔍 CHẤT LƯỢNG (ultracode):** (1) **mutation spike** — phá 5 guard tiền trong route THẬT (CAS chấm, quest double-claim, redeem trust-client-cost, momo skip-signature, momo skip-idempotent) → mỗi cái làm ĐÚNG test tương ứng chuyển ĐỎ → revert. Bằng chứng test KHÔNG vô nghĩa. (2) **workflow audit đối kháng 7 agent** tìm vacuous-test + fidelity-drift → đã vá keystone: deep-clone JSONB (no-op save giờ làm test đỏ, trước là xanh giả), execRpc phát PGRST202 (mã prod thật khi chưa migration, KHÔNG phải 42883), redeem no_row trả coins:0 khớp SQL; VNPay test cũ VÔ NGHĨA (chết ở getVnpayClient throw vì thiếu creds) → viết lại set env test → verify chữ ký THẬT.
> - **⚠️ GIỚI HẠN (ghi rõ `test-int/README.md` — đừng tự-tin-giả):** fake single-threaded KHÔNG kiểm được **race đồng thời thật** (chốt atomic CAS/`FOR UPDATE` chỉ kiểm ở dạng tuần tự; race an toàn đã verify RIÊNG ở SQL + live prod — memory ROOT C/reward-to-real). KHÔNG mô phỏng RLS (verify riêng 1.3). Đừng viết test tự nhận "chống race".
> - **✅ RỦI RO APP phát hiện khi audit → ĐÃ KHOÁ (commit `9ef2e69`, user chọn "khoá route"):** `GET /api/migrate-data` từng upsert coins/xp từ file JSON local — GET + không auth + không rate-limit + ghi ĐÈ = vector faucet xu (§9.6) nếu file hiện diện (prod TRƠ vì gitignored). Vá 4 lớp: (1) **env flag `ENABLE_LEGACY_MIGRATION` mặc định TẮT → 410** (route chết trên prod), (2) auth → 401, (3) rate-limit 3/phút, (4) economy chỉ INSERT khi CHƯA có row (KHÔNG upsert ghi đè). +2 integration test (410/401 deterministic; đường đọc-file không test vì phụ thuộc file legacy có mặt → non-deterministic dev/CI). ⚠️ máy dev HIỆN CÓ file `../10.SAT_Prep_App - Copy/streak_data.json` (bài học: đừng viết test phụ thuộc file legacy). (Critic cũng lưu payment/create amount-integrity — đã có test phủ, an toàn.)
> - **🔍 VERIFY:** tsc sạch · test **238/238** (178 unit + 60 integration) · lint 0/0 · build 59 pages. origin/main HEAD `9ef2e69`.
>
> **⏳ VIỆC PHIÊN SAU (vẫn CHỜ user, y như trước):** (1) 🟡 CỔNG THANH TOÁN — chốt giá 4 gói + creds sandbox VNPay/MoMo → verify roundtrip live. (2) 🟢 ADMIN FULFILLMENT — chọn hướng (a) shared-secret nhẹ / (b) role system. (3) ⏳ Beta 100 users. Secret rotate: vẫn chỉ DB pw đã đổi (GitHub PAT/Vercel/OpenAI chưa). [Tùy chọn Claude tự làm nếu user vẫn defer: khoá `migrate-data`, hoặc thêm test 429 rate-limit.]
