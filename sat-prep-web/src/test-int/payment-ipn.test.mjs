...
// ── A3 — GIA HẠN CỘNG DỒN (extend-from-active-expiry) ────────────────────────
// Bất biến A3: gia hạn CÙNG TIER khi còn hạn → expires_at MỚI = max(now, expiry
// cũ) + duration, KHÔNG reset về now (không mất thời gian đã trả).
test('A3: gia hạn CÙNG TIER khi còn hạn → CỘNG DỒN thời gian (không mất ngày còn lại)', async () => {
  resetDb(); setMomoEnv(true);
  // Gói ultimate hiện có còn hạn RẤT xa (2027) — mô phỏng còn nhiều ngày.
  const farFuture = '2027-01-01T00:00:00.000Z';
  seed('user_subscriptions', {
    id: 'sub-existing', user_id: 'u-a3', tier: 'ultimate', period: 'monthly',
    started_at: '2026-07-01T00:00:00.000Z', expires_at: farFuture, created_at: '2026-07-01T00:00:00.000Z',
  });
  // Gia hạn thêm 30 ngày CÙNG tier ultimate.
  seedPendingTxn('A3-ORD', { userId: 'u-a3', tier: 'ultimate', period: 'monthly', amountVnd: 990000, durationDays: 30 });

  const res = await momoIpn(momoReq(momoPayload('A3-ORD', { amount: 990000 })));
  assert.equal(res.status, 204);

  const subs = getRows('user_subscriptions').filter((s) => s.user_id === 'u-a3');
  assert.equal(subs.length, 2, 'có dòng gói mới');
  const newSub = subs.find((s) => s.id !== 'sub-existing');
  // expires_at MỚI = farFuture + 30 ngày (cộng dồn), KHÔNG phải now + 30 ngày.
  const expectedMs = new Date(farFuture).getTime() + 30 * 86400 * 1000;
  assert.equal(new Date(newSub.expires_at).getTime(), expectedMs, 'cộng dồn từ expiry cũ');
});

test('A3: gia hạn KHÁC TIER → KHÔNG cộng dồn (extend từ now, giữ hành vi cũ)', async () => {
  resetDb(); setMomoEnv(true);
  const farFuture = '2027-01-01T00:00:00.000Z';
  seed('user_subscriptions', {
    id: 'sub-prem', user_id: 'u-a3b', tier: 'premium', period: 'monthly',
    started_at: '2026-07-01T00:00:00.000Z', expires_at: farFuture, created_at: '2026-07-01T00:00:00.000Z',
  });
  // Mua ULTIMATE (khác tier premium đang có) → không cộng dồn từ premium.
  seedPendingTxn('A3-XT', { userId: 'u-a3b', tier: 'ultimate', period: 'monthly', amountVnd: 990000, durationDays: 30 });

  const res = await momoIpn(momoReq(momoPayload('A3-XT', { amount: 990000 })));
  assert.equal(res.status, 204);

  const ult = getRows('user_subscriptions').find((s) => s.user_id === 'u-a3b' && s.tier === 'ultimate');
  const startMs = new Date(ult.started_at).getTime();
  assert.equal(new Date(ult.expires_at).getTime() - startMs, 30 * 86400 * 1000, 'ultimate extend từ now (không dính premium)');
});
