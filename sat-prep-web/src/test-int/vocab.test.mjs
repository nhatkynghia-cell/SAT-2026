...
// REGRESSION (adversarial review wf_4e1ca5c7): user MỚI chưa có row user_economy.
// ensureEconomyRow tạo row TRƯỚC RPC → RPC khóa dòng được → claim-once đúng, KHÔNG
// double-grant (trước đây no_row → fallback không ghi claim → 2 POST cộng đôi).
// 🔴 (review wf_296d4ade) ensureEconomyRow seed DEFAULT_ECONOMY.coins (100 xu chào
// mừng) — KHÔNG phải 0 — khớp loadEconomy cho user rowless → user mới KHÔNG mất 100
// xu welcome khi hành động cộng-xu đầu tiên là ôn từ. coins = 100 + 5 = 105.
test('vocab: user mới (chưa có economy row) → tạo row (100 welcome) + thưởng đúng 1 lần, ghi claim', async () => {
  resetDb(); setCurrentUser({ id: 'v-new' });
  // KHÔNG seedUser → chưa có row user_economy.
  seedVocab('v-new', [{ id: 'w1', box: 1, next_review: '2020-01-01' }]);

  const r = await readRes(await POST(postJson({ wordId: 'w1', isRemembered: true })));
  assert.deepEqual(r.body.granted, { coins: 5, xp: 20 });
  const econ = getRows('user_economy').find((e) => e.user_id === 'v-new');
  assert.ok(econ, 'đã tạo row user_economy cho user mới');
  assert.equal(econ.coins, 105, 'user mới GIỮ 100 xu welcome + 5 thưởng (KHÔNG seed 0)');
  // claim ĐƯỢC ghi → POST trùng sau đó không cộng lại.
  const bucket = econ.quest_claims?.['__vocab_reward__'];
  assert.ok(Array.isArray(bucket) && bucket.some((k) => k.startsWith('w1:')), 'ghi claim ngay lần đầu');
});

// REGRESSION: thưởng chốt TRƯỚC khi advance Leitner → nếu save/thưởng lỗi thì
// next_review CHƯA bị đẩy tương lai → retry vẫn due. Ở đây kiểm thứ tự thành công:
// grant xong thì Leitner MỚI được advance (box tăng + next_review đổi).
test('vocab: thưởng chốt trước, Leitner advance sau (box + next_review cập nhật khi thành công)', async () => {
  resetDb(); setCurrentUser({ id: 'v-order' }); seedUser('v-order');
  seedVocab('v-order', [{ id: 'w1', box: 1, next_review: '2020-01-01' }]);

  const r = await readRes(await POST(postJson({ wordId: 'w1', isRemembered: true })));
  assert.equal(r.body.word.box, 2, 'box advance sau khi thưởng chốt');
  assert.notEqual(r.body.word.next_review, '2020-01-01', 'next_review đã đẩy tương lai');
  assert.equal(getRows('user_economy')[0].coins, 105);
});
